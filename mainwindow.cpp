  ...
  , ui(new Ui::MainWindow)
{
	...
	for (j= 0; j < 8; j++) {
	    ...
	    connect(
	      tabla->celije[i][j], &QTCell::clicked, this, &MainWindow::turn);
	}
    }
    connect(ui->buttonGo, &QPushButton::clicked, this, &MainWindow::bestMove);
    connect(
      ui->buttonNewGame, &QPushButton::clicked, this, &MainWindow::newGame);
}
//! Destructor
...
{
    // Display board
    // We need to make a QT object for this one, but for now it's in the
    // terminal
    std::cout << board;
    // CLANG-TIDY
	...
	for (int col= 1, col2= 8; col <= 8; ++col, --col2) {
		...
		// std::endl;
		this->tabla->celije[row - 1][col2 - 1]->setPicture(
		  absolute + adrese[piece + 6]);
	    }
	}
	std::cout << std::endl;
    }
...
MainWindow::turn(int x, int y)
{
	...
	*/
	...
	this->command=
	  "move " + x1 + std::to_string(y1) + x2 + std::to_string(y2);
	...
	if (this->command.compare(0, 5, "move ") == 0) {
		...
		std::cout << "Try again. Use long notation, e.g. e2e4"
			  << std::endl;
		refreshBoardSelect();
		...
		if (check) {
		    ...
		    std::cout << "You are in CHECK. Play another move."
			      << std::endl;
		    refreshBoardSelect();
		    return;
		}
	    ...
	    std::cout << "Valid commands are: quit, move, go, show"
		      << std::endl;
	    refreshBoardSelect();
	    return;
	}
