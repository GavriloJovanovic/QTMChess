...
CBoard::find_legal_moves(CMoveList& moves) const
{
  ...
  if (m_side_to_move > 0)
	...
	case WK: // White king
	{
	  ...
	  if (!CMove::WRR_moved && !CMove::WK_moved && m_board[F1] == EM &&
	      m_board[G1] == EM && !isKingInCheck() &&
	      !isSquareThreatened(F1) && !isSquareThreatened(G1)) {
	  ...
	  if (!CMove::WRL_moved && !CMove::WK_moved && m_board[B1] == EM &&
	      m_board[C1] == EM && m_board[D1] == EM && !isKingInCheck() &&
	...
	case BK: // Black king
	{
	  ...
	  if (!CMove::BRR_moved && !CMove::BK_moved && m_board[F8] == EM &&
	      m_board[G8] == EM && !isKingInCheck() &&
	      !isSquareThreatened(F8) && !isSquareThreatened(G8)) {
	  ...
	  if (!CMove::BRL_moved && !CMove::BK_moved && m_board[B8] == EM &&
	      m_board[C8] == EM && m_board[D8] == EM && !isKingInCheck() &&
