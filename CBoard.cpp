 ...
 ***************************************************************/
std::ostream&
operator<<(std::ostream& os, const CBoard& rhs)
{
 ...
 ***************************************************************/
void
CBoard::find_legal_moves(CMoveList& moves) const
{
    moves.clear();
	...
	for (int i= A1; i <= H8; ++i) {
		...
		case WK: // White king
		{
		    ...
		    if (!CMove::WRR_moved && !CMove::WK_moved &&
			m_board[F1] == EM && m_board[G1] == EM &&
			!isKingInCheck() && !isSquareThreatened(F1) &&
			!isSquareThreatened(G1)) {
		    ...
		    if (!CMove::WRL_moved && !CMove::WK_moved &&
			m_board[B1] == EM && m_board[C1] == EM &&
			m_board[D1] == EM && !isKingInCheck() &&
			!isSquareThreatened(C1) && !isSquareThreatened(D1)) {
	...
	for (int i= A1; i <= H8; ++i) {
		...
		case BK: // Black king
		{
		    ...
		    if (!CMove::BRR_moved && !CMove::BK_moved &&
			m_board[F8] == EM && m_board[G8] == EM &&
			!isKingInCheck() && !isSquareThreatened(F8) &&
			!isSquareThreatened(G8)) {
		    ...
		    if (!CMove::BRL_moved && !CMove::BK_moved &&
			m_board[B8] == EM && m_board[C8] == EM &&
			m_board[D8] == EM && !isKingInCheck() &&
			!isSquareThreatened(C8) && !isSquareThreatened(D8)) {
 ...
 ***************************************************************/
void
CBoard::make_move(const CMove& move)
{
 ...
 ***************************************************************/
void
CBoard::undo_move(const CMove& move)
{
    m_material= -m_material;
    switch (move.m_captured) {
	case WP:
...
int
CBoard::getSideToMove() const
{
    return m_side_to_move;
}

bool
CBoard::IsMoveValid(CMove& move) const
{
    CMoveList moves;
    find_legal_moves(moves);
	...
	if (moves[i] == move) {
		...
		CMove::WK_moved= true;
	    if (move.m_from == A1 && move.m_piece == WR &&
		CMove::WRL_moved != true)
		CMove::WRL_moved= true;
	    if (move.m_from == H1 && move.m_piece == WR &&
		CMove::WRR_moved != true)
		CMove::WRR_moved= true;
	    if (move.m_piece == BK && CMove::BK_moved != true)
		CMove::BK_moved= true;
	    if (move.m_from == A8 && move.m_piece == BR &&
		CMove::BRL_moved != true)
		CMove::BRL_moved= true;
	    if (move.m_from == H8 && move.m_piece == BR &&
		CMove::BRR_moved != true)
		CMove::BRR_moved= true;

	    if (move.m_piece == WP && move.m_to.row() == 4 &&
		move.m_from.row() == 2) {
		CMove::en_passant_square= move.m_to;
		CMove::en_passant_ready= true;
	    } else if (move.m_piece == BP && move.m_to.row() == 5 &&
		       move.m_from.row() == 7) {
		CMove::en_passant_square= move.m_to;
		CMove::en_passant_ready= true;
	    }

	    else if (move.m_piece == WP && move.m_from.row() == 5 &&
		     move.m_to == CMove::en_passant_square + 10 &&
		     CMove::en_passant_ready) {
		CMove::en_passant_played= true;
	    } else if (move.m_piece == BP && move.m_from.row() == 4 &&
		       move.m_to == CMove::en_passant_square - 10 &&
		       CMove::en_passant_ready) {
		CMove::en_passant_played= true;
	    } else {
 ...
 ***************************************************************/
bool
CBoard::isOtherKingInCheck() const
{
